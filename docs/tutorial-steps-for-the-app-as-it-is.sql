-- The guided tour, rewritten for the app that exists.
--
-- NOT YET APPLIED.
--
-- The ten steps in tutorial_steps describe a note-taking app: audit logs, a
-- notes search, a new-note button. Audit logs and notes were both removed, and
-- touchpoints, forms, billing and the document import - the whole of the work
-- this app now does - appear nowhere in it.
--
-- Every selector below exists in the interface today.
...
-- Expect: employee 4, admin 8.
select role_type, count(*) as steps
  from public.tutorial_steps group by role_type order by role_type;
