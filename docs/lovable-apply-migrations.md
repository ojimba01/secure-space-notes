# Applying the intake and Availity migrations through Lovable

Misky has no Supabase dashboard access, so schema changes go through Lovable's
own agent, the same way edge function deploys do (see `docs/sheet-intake.md`).

Paste everything below the line into Lovable's chat, in one message.

**Two things to know first.**

- This spends Lovable credits. It is one message, so it should be one cheap turn.
- The prompt tells Lovable to run SQL and change nothing else. That matters: its
  agent will otherwise try to build the screens that use these tables, and those
  screens already exist on the `client-intake-form` branch. If it commits a
  migration file to `main` anyway, that is harmless — the same migration is
  idempotent — and it gets reconciled when the branch merges.
...
anything to the app's UI — the screens that use these tables already exist on a
branch and will be merged separately. Run the SQL exactly as written and tell me
the result.
...
After running it, please confirm these four things are true:

1. `public.client_intakes` exists
2. `public.client_intake_household_members` exists
3. `public.clients` has a `medicaid_id` column
4. `public.compliance_settings` has a row with key `availity_provider`
